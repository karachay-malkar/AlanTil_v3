(function () {
  ...
  // Builtin/User dict lists (v5)
  const builtinDictsList = document.getElementById("builtinDictsList");
  const userDictsList = document.getElementById("userDictsList");
  const btnImportUserCsv = document.getElementById("btnImportUserCsv");
  const userCsvInput = document.getElementById("userCsvInput");
...
// Sections
  ...
  function dictDisplayName(code) { return isUserDictName(code) ? fromUserDictName(code) : dictTitle(code); }
  ...
  function renderDicts() {
    const builtinDicts = dictsFrom(DATA_BUILTIN);
    const userDicts = dictsFrom(DATA_USER);

    // Backward compat: if old container exists, keep it empty
    if (dictsList) dictsList.innerHTML = "";

    if (builtinDictsList) {
      builtinDictsList.innerHTML = builtinDicts
        .map(d => `<button class="btn" data-dict="${escapeHtml(d)}">${escapeHtml(dictDisplayName(d))}</button>`)
        .join("");
      builtinDictsList.querySelectorAll("button[data-dict]").forEach(btn => {
    ...
    if (userDictsList) {
      userDictsList.innerHTML = userDicts
        .map(d => `<button class="btn" data-dict="${escapeHtml(d)}">${escapeHtml(dictDisplayName(d))}</button>`)
        .join("");
      userDictsList.querySelectorAll("button[data-dict]").forEach(btn => {
        btn.addEventListener("click", () => {
          currentDict = btn.getAttribute("data-dict");
          renderSections(currentDict);
          showView(viewSections);
...
function renderSections(dict) {
    sectionsTitle.textContent = `Разделы: ${dictDisplayName(dict)}`;
  ...
  function openSetMenu() {
    ...
    setMenuTitle.textContent = `${dictDisplayName(currentDict)} • ${sectionTitle(currentSection)} • Сет ${currentSet}`;
  ...
  function openGlobalTestMenu() {
    ...
    globalDictSelect.innerHTML = [
      ...
      ...dicts.map(d => `<option value="${escapeHtml(d)}">${escapeHtml(dictDisplayName(d))}</option>`)
    ].join("");
  ...
  function updateGlobalTestInfo() {
    ...
    const scopeDict = (dval === "__all__") ? "Все словари" : dictDisplayName(dval);
  ...
  // ---------- User CSV import (same format as server CSV)
  function validateUserCsvWords(words) {
    if (!Array.isArray(words) || !words.length) throw new Error("CSV пустой или неверные заголовки");
    // Must have dict/section/set/word/trans; parseCsv guarantees id/set/word/trans
    return words;
  }

  async function importUserCsvFile(file) {
    const text = await file.text();
    const parsed = parseCsv(text);
    const words = validateUserCsvWords(parsed).map(w => ({
      ...w,
      // Prefix dict to avoid collisions with встроенными
      dict: toUserDictName(w.dict || "Словарь"),
      section: (w.section || w.folder || "Раздел").trim() || "Раздел",
      set: Number(w.set || 0) || 1,
      source: "user",
    }));

    const existing = loadUserWords();
    // Merge: if same user dict name exists, replace it
    const dictNames = uniq(words.map(w => w.dict));
    let merged = existing.filter(w => !dictNames.includes(w.dict));
    merged = merged.concat(words);
    saveUserWords(merged);
    return merged;
  }

  if (btnImportUserCsv && userCsvInput) {
    btnImportUserCsv.addEventListener("click", () => {
      userCsvInput.value = "";
      userCsvInput.click();
    });
    userCsvInput.addEventListener("change", async () => {
      ...
      if (!file) return;
      try {
        const merged = await importUserCsvFile(file);
        DATA_USER = merged;
        DATA = DATA_BUILTIN.concat(DATA_USER);
        renderDicts();
        alert("Словарь загружен ✅");
      } catch (e) {
        alert("Ошибка загрузки CSV: " + (e?.message || e));
      }
    });
  }
...
if (!Array.isArray(DATA) || !DATA.length) {
    ...
    // normalize builtin
    DATA_BUILTIN = (DATA_BUILTIN || []).map(w => ({
      ...w,
      dict: (w.dict || "Базовая лексика").trim() || "Базовая лексика",
      section: (w.section || w.folder || "Раздел").trim() || "Раздел",
      set: Number(w.set || 0) || 1,
      source: "builtin",
    }));

    // load & normalize user words (stored already in same schema as server CSV, but dict is prefixed)
    DATA_USER = loadUserWords().map(w => ({
      ...w,
      dict: (w.dict || toUserDictName("Словарь")).trim() || toUserDictName("Словарь"),
      section: (w.section || w.folder || "Раздел").trim() || "Раздел",
      set: Number(w.set || 0) || 1,
      source: "user",
    }));
