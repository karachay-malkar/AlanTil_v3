...
function resetFlipInstant() {
  // Instantly return the flip-card to the front without showing a "reverse flip" animation
  const inner = card && card.querySelector ? card.querySelector(".cardInner") : null;
  if (!inner) {
    // Fallback: just remove the class
    if (card && card.classList) card.classList.remove("flipped");
    return;
  }
...
card.addEventListener("click", (e) => {
  // Ignore taps on action buttons inside the card area
...
// ---------- Study renderers (v7.3)
  ...
  function updateFavActionUI(){
    ...
    if (favActionLabel) favActionLabel.textContent = on ? "Избранное" : "Сохранить";
    btnFavAction.setAttribute("aria-label", on ? "Убрать из избранного" : "Добавить в избранное");
  }
