(function () {
  ...
  function showView(which) {
    [viewDicts, viewLearnMenu, viewSections, viewSets, viewSetMenu, viewGlobalTestMenu, viewTest, viewStudy, viewDictContent, viewSessionAnalytics].forEach(v => v.classList.add("hidden"));
...
function updateGlobalTestInfo() {
  ...
  function openSessionAnalytics() {
    ...
    if (!problemWords.length) {
      analyticsList.innerHTML = "<div class='smallNote'>Ошибок не было 🎉</div>";
    } else {
      analyticsList.innerHTML = problemWords.map(w => `
        <div class="dictWordRow">
          ...
          <button class="starBtn ${isFav(w.id) ? "on" : ""}">
            ${isFav(w.id) ? "★" : "☆"}
          </button>
        </div>
      `).join("");
